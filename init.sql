...
BEGIN
    ...
    -- Inserir endereço de meio (opcional)
    INSERT INTO address (cep, street, city, state, district, house_number, direction, user_id)
    VALUES ('05001-000', 'Rua Intermediária', 'Campinas', 'SP', 'Centro', '456', 'MIDDLE', 1)
    ...
    -- Criar a origem usando a função create_origin
    v_origin_id := create_origin(v_origin_id, 4);

    -- Criar o meio (opcional) usando a função create_middle
    v_middle_id := create_middle(v_middle_id, 3, 3600, '100 km', v_origin_id);

    -- Criar o destino usando a função create_destination
    v_destination_id := create_destination(v_destination_id, 3600, '200 km', v_origin_id, v_middle_id);

    -- Inserir na tabela travels usando os IDs das tabelas relacionadas
    INSERT INTO travels (status, user_id, renavam, departure, origin_id, middle_id, destination_id, arrival, involved)
    VALUES (TRUE, 1, '98765432109', '2024-09-05 14:00:00', v_origin_id, v_middle_id, v_destination_id, '2024-09-05 17:00:00', '{}');
    
    -- Exibir o resultado da consulta final
    RAISE NOTICE 'Travel successfully created for user with ID 1';
END $$;

SELECT * FROM users WHERE email = 'vitor@email.com';
INSERT INTO Rate (author_id, recipient_id, creation, rate_type, grade, content)
VALUES (1, 1, NOW(), 'CARONISTA', 'OTIMO', 'Motorista excelente e muito educado');